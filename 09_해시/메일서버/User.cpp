...
list<int> user[MAX_USER];
int title[MAX_TITLE];
int word_hash[MAX_TITLE][11];
...
void init(int N, int K) {
	...
	for (int idx = 0; idx < MAX_TITLE; ++idx) {
		for (int idx2 = 0; idx2 < 11; ++idx2) {
			word_hash[idx][idx2] =-1;
		}
	}
...
void sendMail(char subject[], int uID, int cnt, int rIDs[]) {
	for (int idx = 0; idx < cnt; ++idx) {
		int rev = rIDs[idx];
		int mail_count = getCount(rev);

		if (mail_count >= k) {
			user[rev].pop_front();			
		}		
		user[rev].push_back(title_cnt);
	}

	title[title_cnt] = get_hash(subject);
	...
	string substr("");	
	while (iss >> substr) {
		word_hash[title_cnt][idx++] = get_hash(substr);
	}
	++title_cnt;
}
...
int deleteMail(int uID, char subject[]) {
	...
	int val = get_hash(subject);
	for (auto iter = user[uID].begin(); iter != user[uID].end();) {
		int target_hash = title[*iter];
...
int searchMail(int uID, char text[]) {
	int srch_cnt(0);
	int text_hash = get_hash(text);	
	
	for (int idx : user[uID]) {
		for (int idx2 = 0; idx2 < 11; ++idx2) {			
			if (word_hash[idx][idx2] == -1) break;
			else if (word_hash[idx][idx2] == text_hash) {
				++srch_cnt;
				break;
			}
		}		
